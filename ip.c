...
ip_iface_register(struct net_device *dev, struct ip_iface *iface)
{
    ...
    infof("registered: dev=%s, unicast=%s, netmask=%s, broadcast=%s, network=%s", dev->name,
        ip_addr_ntop(iface->unicast, addr1, sizeof(addr1)),
        ip_addr_ntop(iface->netmask, addr2, sizeof(addr2)),
        ip_addr_ntop(iface->broadcast, addr3, sizeof(addr3)),
        ip_addr_ntop(iface->unicast & iface->netmask, addr4, sizeof(addr4))
        );
    return 0;
}
...
ip_output_core(struct ip_iface *iface, uint8_t protocol, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, ip_addr_t nexthop, uint16_t id, uint16_t offset)
{
    ...
    memcpy(hdr+1, data, len); // pointer increment changed by type(int, sturct)
